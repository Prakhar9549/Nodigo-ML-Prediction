...
@app.route('/supervised/svr')
def svr():
    return render_template('/supervised/svr/svr.html')

@app.route('/supervised/svr',  methods=['GET', 'POST'])
def svroutput():
    if request.method == 'POST':
        my_dataset = request.files['my_dataset']
        my_model_name = request.form['name_of_model']
        ...
        slope = svr.intercept_
        training_score = svr.score(X_train, y_train)
        testing_score = svr.score(X_test, y_test)
        ...
        print("[INFO] Saving model...")
        # Save the trained model as a pickle string. 
        saved_model=pickle.dump(svr,open("static/data-preprocess/model/model.pkl", 'wb')) 
    
        return render_template('/supervised/svr/svroutput.html', dataset_name=my_dataset.filename, 
                               model_name=my_model_name,var1=intercept,var2=slope, visualize=get_plot1,
                               data_shape=df.shape, 
                               table=df.head(5).to_html( classes='table table-striped table-dark table-hover x'), dataset_describe=df.describe().to_html(classes='table table-striped table-dark table-hover x'), trainingscore=training_score, testingscore=testing_score, first_col=col_one, sec_col=col_last,)
...
#@app.route('/supervised/regression/decisionTreeRegression')
#def regressionLR():
#     return render_template('/supervised/regression/linearRegression.html')
